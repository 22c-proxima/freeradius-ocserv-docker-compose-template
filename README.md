# VPN-платформа на базе FreeRADIUS и OpenConnect. Учебное пособие/шаблон
...
- [`.ci`](.ci) — создание контейнера для тестирования подключений этому VPN.
- [`freeradius/`](freeradius/) — конфигурация и скрипты FreeRADIUS.
- [`gen-cert/`](gen-cert) — генерация сертификата и подготовка скрипта тестирования.
...
- Используйте `docker compose exec <название_сервиса> /bin/sh` чтобы исполнять произвольные команды на работающем сервисе
